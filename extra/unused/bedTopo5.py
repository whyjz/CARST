...
def bedTopo(east_grd_path, dem_grd_path, thickness_txt_path):
	...
	dist_grd_path   = east_grd_path.replace("eastxyz", "dists");
	duxdy_grd_path  = east_grd_path.replace("eastxyz", "duxdy");
...
#	f        = netcdf.netcdf_file("ice_only.grd","r",False);
	...
	f      = netcdf.netcdf_file(dist_grd_path,"r",False);
	x      = f.variables["x"].data;
	y      = f.variables["y"].data;
	dists  = f.variables["z"].data[:];
	f.close();

	f      = netcdf.netcdf_file(duxdy_grd_path,"r",False);
	x      = f.variables["x"].data;
	y      = f.variables["y"].data;
	duxdy  = abs(f.variables["z"].data[:]);
	f.close();
...
#	Read in thicknesses, initialize fluxes
	...
	for line in infile:
		...
		if thickness.lower().find("a") > -1 or scipy.isnan(speeds[int(i), int(j)]):
			continue;
...
#	cell_angles  = scipy.flipud(scipy.array([[3 * math.pi / 4, 1 * math.pi / 2, 1 * math.pi / 4], [math.pi, scipy.nan, 0], [-3 * math.pi / 4, -1 * math.pi / 2, -1 * math.pi / 4]]));
	...
	while cur_iteration < max_iterations:
		...
		for coord in current_cells:
			...
			if i < 1 or j < 1 or j == (width - 1) or i == (length - 1):
				continue;
			...
			in_dists  = dists[i-1:i+2,j-1:j+2] * scipy.ones((3,3));
			in_duxdy  = duxdy[i-1:i+2,j-1:j+2] * scipy.ones((3,3));

			in_speeds[1,1] = scipy.nan;
			in_angles[1,1] = scipy.nan;
			in_dists[1,1]  = scipy.nan;
			in_duxdy[1,1]  = scipy.nan;
			in_locked[1,1] = scipy.nan;

			out_flux  = fluxes[i,j];

			in_fluxes = getFluxes(in_speeds, in_angles, in_dists, in_duxdy, out_flux, inc);
		...
		for coord in todo_cells:
			...
			for k in range(0, in_fluxes.shape[0]):
				...
				for m in range(0, in_fluxes.shape[1]):
					...
					print(in_fluxes[k,m]);
					fluxes[i+(k-1),j+(m-1)] += 1 * in_fluxes[k,m];
					fluxes[i,j]             -= 1 * in_fluxes[k,m];

#		print(old_out_flux, new_out_flux);

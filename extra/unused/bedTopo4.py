	...
	while cur_iteration < max_iterations:
		...
		while len(current_cells) > 0:
			...
			for coord in current_cells:
				...
				temp_angles                    = angles[i-1:i+2,j-1:j+2] * scipy.ones((3,3)) * locked[i-1:i+2,j-1:j+2];
				temp_angles[1,1]               = scipy.nan;
				in_angles                      = scipy.cos(cell_angles - temp_angles);
	        		in_angles[in_angles < 0.00001] = scipy.nan;

				count = sum(sum(~scipy.isnan(in_angles)));

				if count not in counts:
					counts[count] = [coord];
				else:
					counts[count].append(coord);

			sorted_counts = sorted(counts.keys());

			for coord in counts[sorted_counts[0]]:
				...
				for k in range(0, in_fluxes.shape[0]):
					...
					for m in range(0, in_fluxes.shape[1]):
						...
						tolock[in_coord] = True;
						todo_cells[in_coord] = True;

				del current_cells[coord];

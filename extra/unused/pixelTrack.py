...
def adjustPhase(fileName,wavelength,width):
 ...
 radarUNWData = scipy.matrix(numpy.fromfile(radarUNWFile,numpy.float32,-1)).reshape(width,-1);
...
def ampcor(path,rwin,awin,wsamp,numproc):
 ...
 for i in range(0,len(cullFindPaths)):
  ...
  findSlc1Stream.close();
  findSlc2Paths = findSlc2Output.split();
  ...
  ampCmd+="\ncat ";
  for i in range(1,int(numproc)+1):
   ampCmd+=cullFileName[index1:index4] + "_azo_" + wsamp + "_"+rwin+"x"+awin+"_"+str(i)+".off ";
  ampCmd+=" > "+cullFileName[index1:index4] + "_azo_" + wsamp + "_"+rwin+"x"+awin+".off\n";
  ampCmd += "\ncd " + currentDir + "\n";
...
def makeUNW(path,rwin,awin,wsamp):
 findAmpcorInCmd = "\nfind " + path + " -name \"*"+rwin+"x"+awin+".off\" -print\n";
 ...
 for i in range(0,len(findAmpcorInOutput)):
  ...
  while 1:
   ...
   if not line:
    ...
    outputLoadMakeAzoFile.write(line.replace("\"\";","\""+ampcorName+".off\";"));
    break;
   else:
    outputLoadMakeAzoFile.write(line);
  while 1:
...
if start == "setup":   
 ...
 for i in range(0,len(leaderFiles)):
  ...
  if re.search("envisat",DataType.lower()):
   ...
   findRawStream = os.popen(findRawCmd);
   findRawOutput = findRawStream.read().strip().split();
   findRawStream.close();
   rawFileName = findRawOutput[0];
   if len(findRawOutput) > 1:
    hvFileName=findRawOutput[1];
  ...
  if not os.path.exists(tempImagePath):
   ...
   if hvFileName!="":
    tempImagePath = RawRawFolder + "/" + dateName + "/" + hvFileName[hvFileName.rfind("/")+1:].strip();
    os.symlink(hvFileName,tempImagePath);
 start = "make_raw";
 if start == endStep:
  logFile = open("pixelTrackLog.txt","a");
  logFile.write("\n***** pixelTrack.py for parameters \""+parameterFilePath+"\" - Started at \""+startStep+"\", ended at \""+endStep+"\", time elapsed to completion: " + ("%.3f seconds\n" % (time.time()-t1)));
  ...
  if re.search("^\d\d\d\d\d\d$",rawRawFolderContents[i]):
   ...
   for j in range(0,len(dateFolderContents)):
    ...
    if dateFolderContents[j].find(findStr) > -1:
     ...
     if re.search("alos",DataType.lower()):
      yes_no_maybe_so="NO";
      find_hv_cmd="\nfind "+RawRawFolder + "/" + rawRawFolderContents[i]+" -name \"*IMG*HV*\" -print\n";
      pipe=subprocess.Popen(find_hv_cmd,shell=True,stdout=subprocess.PIPE).stdout;
      hvs=pipe.read().strip();
      pipe.close();
      if hvs.find("HV") > -1:
       yes_no_maybe_so="FBD2FBS";
      makeRawCmd = makeRawCmd + "\ncd " + RawRawFolder + "/" + rawRawFolderContents[i] + "\nperl " + ROIPAC + "/" + makeRawScript + " IMG-HH-ALP " + rawRawFolderContents[i] + " "+yes_no_maybe_so+"\ncd " + currentDir + "\n";
     elif re.search("envisat",DataType.lower()):

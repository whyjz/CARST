...
# ---- configfile example ----
#
# ampcor_prefix   = ampcor_r125x125_s25x20
# output_prefix   = 23JUL15_07JUL15_SEVZ_VavilovW_res30m_hp300
# input_image     = LC81690022015204LGN00_B8_hp300.img
# output_cellsize = 45
# datedelta       = 1.066666667
# mean_x_off      = 0
# mean_y_off      = 0
#
# ----------------------------
...
#from configfile read mean_x_off and mean_y_off instead of from ${ampcor_prefix}_1.in
# mean_x_off=$(grep "Mean Offset" ${ampcor_prefix}_1.in | cut -d "=" -f 2 | awk '{print $1}')
# mean_y_off=$(grep "Mean Offset" ${ampcor_prefix}_1.in | cut -d "=" -f 2 | awk '{print $1}')
...
awk -v ul_x=$ul_x -v ul_y=$ul_y -v inc=$inc -v mean_x_off=$mean_x_off -v mean_y_off=$mean_y_off \
   '{OFMT = "%f";  print ul_x + ($1- 1) * inc, ul_y - ($3- 1) * inc, $2 - mean_x_off, $5, $6}' \
   ${ampcor_prefix}.off > ${output_prefix}_eastxyz.txt

awk -v ul_x=$ul_x -v ul_y=$ul_y -v inc=$inc -v mean_x_off=$mean_x_off -v mean_y_off=$mean_y_off \
   '{OFMT = "%f";  print ul_x + ($1- 1) * inc, ul_y - ($3- 1) * inc, $4 - mean_y_off, $5, $7}' \

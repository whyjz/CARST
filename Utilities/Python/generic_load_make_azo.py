...
def load_and_make_azo(): 
    ...
    outfile = open("azimuth_" + str(rwin) + "x" + str(awin) + "_" + str(int(rwin)/int(wsamp)) + "rlks.unw", 'wb');
    outg = scipy.matrix(outg,scipy.float32);
    outg.tofile(outfile);
    outfile.close();

    outfile = open("range_" + str(rwin) + "x" + str(awin) + "_" + str(int(rwin)/int(wsamp)) + "rlks.unw", 'wb');

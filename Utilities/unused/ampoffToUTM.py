...
def ampoffToUTM(ampoff_path, ampin_path, ref_hdr_path, output_label):
	...
	lr_x = str(float(ul_x) + (int(width) / int(skip) - 1) * int(skip));
	lr_y = str(float(ul_y) - (int(length) / int(skip) - 1) * int(skip));
	...
	for line in infile:
		...
		x_pos = str(float(ul_x) + (((int(x_ind) - 1) * int(inc)) / int(skip)) * int(skip));
		y_pos = str(float(ul_y) - (((int(y_ind) - 1) * int(inc)) / int(skip)) * int(skip));

		ew_txt_file.write(x_pos + " " + y_pos + " " + str(float(x_off) - float(mean_x_off) * float(inc)) + " " + snr + "\n");
		ns_txt_file.write(x_pos + " " + y_pos + " " + str(float(y_off) - float(mean_y_off) * float(inc)) + " " + snr + "\n");

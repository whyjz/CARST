...
#			Calculate input fluxes
			...
			in_ice    = ice_vals[i-1:i+2,j-1:j+2];
			...
			for k in range(0, in_fluxes.shape[0]):
				...
				for m in range(0, in_fluxes.shape[1]):
					...
					key = str(i+(k-1)) + " " + str(j+(m-1));

					if scipy.isnan(in_fluxes[k,m]):
						continue;

					if ~scipy.isnan(locked[i+(k-1),j+(m-1)]):
						continue;

					fluxes[i+(k-1),j+(m-1)] += in_fluxes[k,m];
					fluxes[i,j] -= in_fluxes[k,m];
					todo_cells[key] = True;
...
#			Calculate input fluxes
			...
			for k in range(0, in_fluxes.shape[0]):
				...
				for m in range(0, in_fluxes.shape[1]):
					...
					if scipy.isnan(in_fluxes[k,m]) or scipy.isnan(locked[i+(k-1),j+(m-1)]) or key in thicknesses:
						continue;
...
#		if str(x[int(str_j)]) == "549321.194032" and str(y[int(str_i)]) == "6490293.09879":
#			print(fluxes[i-1:i+2,j-1:j+2]);

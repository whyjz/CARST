...
def load_and_make_azo(): 
    ...
    #width0  = max(x1ind)+rwin
    #length0 = max(y1ind)+awin
    ...
    good = (scipy.matrix(scipy.unique1d(scipy.asarray(scipy.concatenate((snrgood, c11good, c22good),axis=0))))).conj().transpose();
    ...
    outfile = open("azimuth_10rlks.unw", 'wb');
    outg = scipy.matrix(outg,scipy.float32);
    outg.tofile(outfile);
    outfile.close();

    outfile = open("range_10rlks.unw", 'wb');

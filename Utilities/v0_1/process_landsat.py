	...
	for pair in pairs:
		...
		for line in infile:
			...
			if line.find("Reference Image Input File") > -1:
				...
				hdr_file = open(elements[len(elements) - 1] + ".hdr");
		...
		if not os.path.exists(n_grd):
			cmd = "";
			cmd += "\ngawk '$4 !~ /a/ {print $1\" \"$2\" \"$4}' " + pair_dir + "/" + pair_label + "_northxyz.txt | xyz2grd -I" + str(int(input_res) * int(rwin)) + "= -G" + pair_dir + "/" + pair_label + "_snr.grd -R" + bounds + "\n"; 

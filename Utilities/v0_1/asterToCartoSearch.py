...
#	Set up search DEM for pc_align
	...
	cmd += "\ngdal_translate -of GMT temp_search.tif temp_search.grd\n";
	cmd += "\ngrdclip temp_diff.grd -Sb-75/NaN -Sa75/NaN -Gtemp_diff.grd\n";
	subprocess.call(cmd, shell=True);

#	if ref_at_search_res_tif == "temp_ref_at_search_res.tif":
#		os.remove(ref_at_search_res_tif);

	removeTrendDEM("temp_search.grd", "temp_diff.grd");

	cmd  = "";
#	cmd  = "\ngrdmath temp_search_rr.grd " + ref_at_search_res_grd + " SUB = temp_diff.grd\n";
#	cmd += "\ngrdclip temp_diff.grd -Sa50/NaN -Sb-50/NaN -Gtemp_diff.grd\n";
#	cmd += "\ngrdmath temp_search_rr.grd temp_diff.grd OR --IO_NC4_CHUNK_SIZE=c = temp_search_rr.grd\n";
	cmd += "\ngdalwarp -of GTiff -t_srs '+proj=utm +datum=WGS84 +zone=" + search_zone + " +" + search_ns + "' -srcnodata \"nan\" -dstnodata \"nan\" temp_search_rr.grd temp_search_rr.tif\n";
	subprocess.call(cmd, shell=True);

#	os.remove(ref_at_search_res_grd);
	os.remove("temp_search.grd");
	os.remove("temp_search_rr.grd");
...
#		cmd  = "\n/home/akm26/Public/StereoPipeline-2.4.2-2014-10-06-x86_64-Linux-GLIBC-2.5/bin/pc_align --compute-translation-only --max-displacement 120 --highest-accuracy --save-transformed-source-points -o temp_search_aligned temp_ref.tif temp_search_rr.tif\n";
		cmd  = "\n/home/akm26/Public/StereoPipeline-2.4.2-2014-10-06-x86_64-Linux-GLIBC-2.5/bin/pc_align --outlier-ratio 0.75 --max-displacement 120 --highest-accuracy --save-transformed-source-points -o temp_search_aligned temp_ref.tif temp_search.tif\n";
	...
	geoTiffDiff(ast_tif_path, ref_at_search_res_tif, ".", "temp_diff.tif", NO_DATA);

	cmd  = "\ngdal_translate -of GMT -a_nodata \"nan\" " + ast_tif_path + " " + ast_grd_path + "\n";
	cmd += "\ngdal_translate -of GMT -a_nodata \"nan\" temp_diff.tif temp_diff.grd\n";
	cmd += "\ngrdclip temp_diff.grd -Sa75/NaN -Sb-75/NaN -Gtemp_diff.grd\n";
	subprocess.call(cmd, shell=True);

	removeTrendDEM(ast_grd_path, "temp_diff.grd");

	cmd  = "\ngdalwarp -of GTiff -t_srs '+proj=utm +zone=" + search_zone + " +datum=WGS84 +" + search_ns + "' -srcnodata \"nan\" -dstnodata \"nan\" " + ast_rr_grd_path + " " + ast_rr_tif_path + "\n"; 
	cmd += "\ngrdmath " + ast_grd_path + " " + ref_at_search_res_grd + " SUB --IO_NC4_CHUNK_SIZE=c = temp_rr_diff.grd\n";
	subprocess.call(cmd, shell=True);

...
def hiResCoreg(ref_dem_tif_path, search_dem_tif_path):
...
#	search_name = "20130307202054"; 
#	search_name = "20130618195812"; 
	search_name = "20131205204849"; 
...
#	print(xmin, xmax, ymin, ymax);
	...
	diff_before_tif_path  = "wv_" + search_name + "_diff_before.tif";
	diff_before_grd_path  = "wv_" + search_name + "_diff_before.grd";
	diff_before_hist_path = "wv_" + search_name + "_diff_before_hist.ps";
...
#	os.remove("temp1.tif");
	...
	ast_pc_path  = "wv_" + search_name + "_srtm_aligned_PC.tif";
	ast_tif_path = "wv_" + search_name + "_srtm_aligned_DEM.tif";
...
#		cmd  = "\n/home/akm26/Public/StereoPipeline-2.4.2-2014-10-06-x86_64-Linux-GLIBC-2.5/bin/pc_align --max-displacement 300 --highest-accuracy " + ref_dem_tif_path + " temp_search_nans.tif -o temp_search_aligned --save-transformed-source-points\n";
		cmd  = "\n/home/akm26/Public/StereoPipeline-2.4.2-2014-10-06-x86_64-Linux-GLIBC-2.5/bin/pc_align --max-displacement 100 --highest-accuracy temp_search_nans.tif " + ref_dem_tif_path + " -o temp_search_aligned --save-inv-transformed-reference-points\n";
		cmd += "\n/home/akm26/Public/StereoPipeline-2.4.2-2014-10-06-x86_64-Linux-GLIBC-2.5/bin/point2dem --nodata-value NaN --t_srs '+proj=utm +datum=WGS84 +zone=" + ref_zone + " +" + ref_ns + "' temp_search_aligned-trans_reference.tif\n";
		cmd += "\ngdalwarp -of GTiff -te " + xmin + " " + ymin + " " + xmax + " " + ymax + " -tr " + ref_res + " " + ref_res + " -srcnodata \"nan\" -dstnodata \"nan\" temp_search_aligned-trans_reference-DEM.tif " + ast_tif_path + "\n";
		subprocess.call(cmd, shell=True);

		os.rename("temp_search_aligned-trans_reference.tif", ast_pc_path);
		os.remove("temp_search_aligned-trans_reference-DEM.tif");

	diff_after_tif_path  = "wv_" + search_name + "_diff_after.tif";
	diff_after_grd_path  = "wv_" + search_name + "_diff_after.grd";
	diff_after_hist_path = "wv_" + search_name + "_diff_after_hist.ps";
	...
	outfile = open("wv_" + search_name + "_srtm_aligned_stats.txt", "w");
...
#	Fit search DEM to reference grid
	...
	ast_diff_tif_path         = "wv_" + search_name + "_srtm_diff.tif";
...
if __name__ == "__main__":
	...
	assert len(sys.argv) > 2, "\n***** ERROR: hiResCoreg.py requires 2 arguments, " + str(len(sys.argv) - 1) + " given\n";
	assert os.path.exists(sys.argv[1]), "\n***** ERROR: " + sys.argv[1] + " does not exist\n";
	assert os.path.exists(sys.argv[2]), "\n***** ERROR: " + sys.argv[2] + " does not exist\n";
	
	hiResCoreg(sys.argv[1], sys.argv[2]);
